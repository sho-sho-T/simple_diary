...
import { getDiaryEntryById, updateDiaryEntry, deleteDiaryEntry } from '../_lib/diary-service';
...
export async function PUT(
  ...
  try {
    ...
    // 日記エントリー更新
    try {
      const updatedEntry = await updateDiaryEntry(authResult.userId, params.id, data);
    ...
    const message = error instanceof Error ? error.message : '日記エントリーの更新に失敗しました';
...
export async function DELETE(
  ...
  try {
    ...
    try {
      ...
      return createSuccessResponse({ message: '日記エントリーが削除されました' });
    } catch (error) {
      // エントリーが存在しない場合
      if (error instanceof Error && error.message.includes('見つかりません')) {
    ...
    const message = error instanceof Error ? error.message : '日記エントリーの削除に失敗しました';
    return createErrorResponse(message, 500);
  }
} 