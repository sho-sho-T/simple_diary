 ...
 * 全ての日記エントリーを取得する
 */
export async function getAllDiaryEntries(userId: string, limit = 10, page = 1): Promise<DiaryEntryWithTags[]> {
 ...
 * 指定した日付の日記エントリーを取得する
 */
export async function getDiaryEntryByDate(userId: string, date: Date): Promise<DiaryEntryWithTags | null> {
 ...
 * 日記エントリーをIDで取得する
 */
export async function getDiaryEntryById(userId: string, id: string): Promise<DiaryEntryWithTags | null> {
  return prisma.diaryEntry.findFirst({
    where: {
      id,
      userId,
...
export async function createDiaryEntry(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existingEntry) {
      throw new Error(`該当日(${data.entryDate.toISOString().split('T')[0]})の日記エントリーは既に存在します`);
    }
 ...
 * 日記エントリーを削除する
 */
export async function deleteDiaryEntry(userId: string, id: string): Promise<DiaryEntry> {
  // エントリーの存在と所有権を確認
 ...
 */
export async function getSimilarDateEntries(userId: string, date: Date, limit = 5): Promise<DiaryEntryWithTags[]> {
